...
class Window(Gtk.Window):
    ...
    def __init__(self):
        ...
        self.lcd.lcd_clear()
        self.lcd.lcd_display_string("   Please, log in   ", 2)
        self.lcd.lcd_display_string("with university card", 3)
        
    def getInfoFromServer(self, widget):
        #Creem la clase ReadDataServer
        server = ReadDataServer(self.uid)
        #Creem el thread lector de dades del server
        #Li assignem server.sendQuery com a run del thread
        #widget.get_text() és la el input de teclat del usuari
    ...
    def changeLabelUsername(self, uid):
        ...
        self.uid = uid
        server = ReadDataServer(self.uid)
        name=server.getName(self.uid)
        self.nameLabel.set_text(name)
        nomsep=name.split(" ")
        self.lcd.lcd_clear()
        self.lcd.lcd_display_string(" Welcome,           ", 2)
        self.lcd.lcd_display_string(" "+nomsep[0]+" "+nomsep[1], 3)
